...
$('#table-cart').on('click', 'a.quitar', function () {
  ...
  $('#guardarVenta').on('click', function () {
    ...
    if (tipoVenta == 2) { // Apartado
      if (pagacon <= 0 || isNaN(pagacon)) {
        showMessage('error', 'El monto recibido debe ser mayor a cero para apartados.');
        return;
      } else {
        abono = pagacon;
        saldo = cambio;
      ...
      if (pagacon < total || isNaN(pagacon)) {
        showMessage('error', 'El monto recibido debe ser mayor o igual al total para ventas.');
        return;
      } else {
        saldo = 0;
      }

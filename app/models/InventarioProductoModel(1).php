...
class InventarioProductoModel
{
  ...
  {
    $sql = "SELECT ip.stock, s.nombre AS nombre_sucursal, p.codigo, p.nombre AS nombre_producto, p.id_producto
      FROM $this->tabla ip
      INNER JOIN sucursales s ON ip.id_sucursal = s.id_sucursal
      INNER JOIN productos p ON ip.id_producto = p.id_producto
  ...
  {
    $sql = "SELECT ip.id_producto, ip.id_sucursal, ip.stock, p.barras, p.codigo, p.nombre, p.precio
          FROM $this->tabla ip
